...
# ### new data --------------------------------------------------
# dat0 <- read_csv(paste0(get(my_computer), "MBI_master_data.csv"))
# studies0 <- dat0 %>% distinct(authyear) %>% flatten_chr()
# dat <- read_csv(paste0(get(my_computer), "new_ES.csv")) %>%
#   mutate(d = as.numeric(d),
#          old_ES = if_else(authyear %in% studies0, 1, 0),                                          # add old ES indicators
#          RCPTHsi = if_else(RCPTHsi < 0, NA_real_, RCPTHsi),
#          HIAGRtot = if_else(HIAGRtot < 0, NA_real_, HIAGRtot)) %>%
#   naniar::replace_with_na_all(condition = ~.x %in% c(-999,-9999,-99999,"-999", "-9999"))
# ps_scalar <- read_csv(paste0(mac2, "PS_scalar_new.csv"))
# # -------------------------------------------------------------
...
# aux <- read.csv(paste0(get(my_computer), "new_ES+Aux.csv"))                                    # data of 1st revision
### ---
...
# write_csv(dat2, "~/Box Sync/Zone/7_MBI_Database/MBX Stuff to Share/dat2_rev1.csv")     # data for propensity-score model in 1st revision
