...
### About v5
This version corresponds to v4 MBI MA manuscript.

* Further info about variable 'T1' from Marta: Not all the reports give us such info, but we try to obtain it. You should count from the day of the intervention, or from the last day of the intervention, through the day of the next measurement (maybe 3 months, then 90 days). We did not use to count from the baseline day because not all the participants completed the intervention.   

### Data pre-proc  - UPDATED
```{r preproc, echo=F}
rm(list=ls())
library(metafor)
library(robumeta)
...
dat <- read.csv(paste0(mac, "Zone/7_MBI_Database/MBX Stuff to Share/CompleteES_May2021.csv"))
target <- read.csv(paste0(mac, "Zone/7_MBI_Database/MBX Stuff to Share/TARGETED.csv"))
# ps_scalar <- read_csv(paste0(mac, "Zone/PS_scalar.csv"))                                      # propensity score
dat <- dat %>%
...
# dat <- dat %>%
#         left_join(ps_scalar, by = "ID")                                                       # add propensity score to the data
...
dat$exclude <- 0
dat$exclude[dat$authyear  == 127] <- 1 #not actually MBX ("Jemmott, Jemmott, Fong & Morales, 2010 (updated)") -
dat$exclude[dat$studyid  == 176] <- 1 #not actually mbx ("Mai 2018 (Completed)") - Mai 2018 (Completed) (fall22-check)
dat$exclude[dat$studyid  == 204] <- 1 #no rec ("Niiranen, 2014") - Niiranen, 2014 (fall22check - new codes)
dat$exclude[dat$studyid  == 315] <- 1 # ("Writing group of the PREMIER collaborative research group, 2003 & Lin et al. 2007 COMPLETED")
...
g <- tally(group_by(dat,ID))                                                                                                     
g <- g %>% rename(numberofrows = n)
dat <-left_join(dat, g, by = "ID")
rm(g)
...
aux <- read.csv(paste0(win, "Zone/7_MBI_Database/MBX Stuff to Share/CompleteES+Aux_May2021.csv"))
...
distress_wide <- distress_wide[-c(4:9)]                                                         # remove distress variables
...
aux_d$elaborative_d <- rowMeans(aux_d[,3:6],na.rm=TRUE)                                 # aggregate distress, motiv, info, skill into 'elaborative'
aux_d$elaborative_d[is.nan(aux_d$elaborative_d)] <- NA
aux_d$info_wellbeing <-rowMeans(aux_d[,c(3,5)],na.rm=TRUE)
...
dat_fp$fptime[dat_fp$wave_base1 == "w6"] <- dat_fp$T5[dat_fp$wave_base1 == "w6"] 
...
# find what stubs are used for clinical vs. behavioral outcomes
# dat %>% 
#         # select(varname, clinical) %>% 
...
# write_csv(dat2, "~/Box Sync/Zone/7_MBI_Database/MBX Stuff to Share/dat2.csv")

## check inclusion criteria in 2022 database update
# dat2 %>%
#   select(authyear, domain, NMainrecom, recommend, reference) %>%
#   mutate(NMainrecom = as.numeric(NMainrecom)) %>%
#   distinct(authyear, .keep_all = T) %>%
#   arrange(domain, desc(NMainrecom), recommend, authyear, reference) %>%
#   write_csv("~/Desktop/MBI_meta_domain_rec.csv")
```
...
sex <- dat[which(dat$varname == "CUSE" | dat$varname == "SEX" | dat$varname == "SEXC" | dat$varname == "ABS" | dat$varname == "CONPRO" | dat$varname == "HIVA"
                 | dat$varname == "HIVT" | dat$varname == "HIVTX"),]
...
summary(wb_results, fit.measures = T, rsquare = T, standardized = T)  # indirect effect is sig. w/ consistent results using different SE calculations
